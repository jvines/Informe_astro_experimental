...
def ap_phot(stamp, cy, cx, ap, sky1, sky2):
    ...
    dist = distance(y_index[cy], x_index[cx], Y, X)
    Q = 2  # ganancia

    star = sp.where(dist <= ap)
    N_a = len(star[0])**2
    flux = stamp[star]

    sky = sp.where((dist > sky1) & (dist < sky2))
    N_b = len(sky[0])**2
    sky_flux = sp.median(stamp[sky])
    sky_err = sp.std(stamp[sky])
...
flx, err = ap_phot(stamp, cy, cx, 7, 15, 20)
